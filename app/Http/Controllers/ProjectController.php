...
//    $total_mass_emission_avoidance = TrialAssement::sum('emission_avoidance');
   ...
   return "Your Preassement is Successful";
    ...
    public function profile(Request $request){
     ...
     public function offsetbids(Request $request){
        ...
        $offsetterspayments = OffsettersTransaction::where('owner_id',$theproid->user_id)->orderBy('id', 'DESC')->simplePaginate(6);

        return view('user.offsetbids',compact('profile','profile_lister','offsetterspayments'));
     }
   ...
   public function searchProject(Request $request)
   {
    ...
    {
       $categories = Project::where('status',2)->orderBy('id', 'DESC')->take(6)->get();
    ...
    public function searchCat(Request $request)
   {
    ...
    $categories = Project::where('status',2)->take(6)->get();

      if($request->categories != ''){

         foreach( $request->categories as $d){

              $categories = Project::where('project_type',$d)->where('status',2)->get();
              $d = $categories;
         }
    ...
    public function searchFilter(Request $request){
        ...
        if($dd != ''){
            ...
            $filter = Project::where('status',2)
                               ->where('carbon_credit','>=',$dd)
                               ->where('carbon_credit','<=',6000)
                               ->get();

            // $filter = Project::where('status',2)
            //                    ->where('carbon_credit','>=',100)
            //                    ->where('carbon_credit','<=',$dd)
            //                    ->get();
            ...
            //     //  $filter = Project::where('status',2)
            //     //                ->where('carbon_credit','>=',0)
            //     //                ->where('carbon_credit','<=',500)
            //     //                ->get();
    ...
    public function updatelog(Request $request){
     ...
     public function previewproject(Request $request,$id){
        ...
        //  $offsetterspayments = Transaction::all();
         ...
         $offsetterspayments = OffsettersTransaction::where('owner_id',$preview->user_id)->where('project_id',$id)->orderBy('id', 'DESC')->get();
         $offsetterspayment = OffsettersTransaction::where('owner_id',$preview->user_id)->where('project_id',$id)->orderBy('id', 'ASC')->get();
         $allprojects = Project::where('user_id',$preview->user_id)->orderBy('id', 'DESC')->take(4)->get();
         $total_offsetbids_user_project = OffsettersTransaction::where('owner_id',$preview->user_id)->where('project_id',$id)->sum('amount');
    ...
    $theowner = DB::table('project')
    ->where('id', $projectid)
    ->first();

    $owner_id = $theowner->user_id;


    $cert_id = chunk_split($transactionid, 3, '-');
        // $repl = preg_replace('/-[^-]*$/', '', $transactionid);

        function removeLastt($str){
    ...
    OffsettersTransaction::create([
        ...
        // $offsetters_unit = Project::where('id',$projectid)->pluck('unit');

        $offsetters_initial_amount = Project::where('id',$projectid)->pluck('offsetters_amount');

        foreach ($offsetters_initial_amount as $key => $value) {

            $offset_value_balance = $value;
        }
        
        $total_offsetvalue = $offset_value_balance+$the_amount;

        $projectinfos = Project::where('id',$projectid)->pluck('tonnes');

        foreach($projectinfos as $projectinfo){
            
            $projectinfotonnes = $projectinfo;
        }

           $balance = $projectinfotonnes-$unit;

        Project::where('id',$projectid)->update(['offsetters_amount' => $total_offsetvalue,'tonnes'=>$balance]); 

        //update offsetter information
        // OffsettersTransaction::where('project_id',$projectid)->update(['unit' => $total_offset_unit_balance]);
